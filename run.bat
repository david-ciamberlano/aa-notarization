...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f alf-algo-share
	call %MVN_EXEC% clean package -pl alf-algo-share,alf-algo-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill alf-algo-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f alf-algo-acs
	call %MVN_EXEC% clean package -pl alf-algo-integration-tests,alf-algo-platform,alf-algo-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl alf-algo-platform,alf-algo-integration-tests,alf-algo-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl alf-algo-platform,alf-algo-integration-tests
EXIT /B 0
:purge
